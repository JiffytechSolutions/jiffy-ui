...
const ImageMagnifier = ({
  ...
  const [magnifierDimension, setMagnifierDimension] = useState({ height: 50, width: 50 })
  ...
  const handelMouseWheel = (e: WheelEvent) => {
    ...
    else setMagnifierZoomLevel(prev => prev - 0.5 >= 1.5 ? prev - 0.5 : 1.5)
  }
  ...
  const getPosition = (): { top: number, left: number } => {
    ...
    const positionObj = {
      left: { top: parentDimension.top, left: (parentDimension.left - (parentDimension.width + 10)) },
      right: { top: parentDimension.top, left: parentDimension.right + 10 },
      top: { top: parentDimension.top - (parentDimension.height + 10), left: parentDimension.left },
      bottom: { top: parentDimension.bottom + 10, left: parentDimension.left }
    }
    if (position !== 'auto') return positionObj[position]

    else {
      if (window.innerWidth - (parentDimension.right + 10) > parentDimension.width) return positionObj["right"]
      else if (parentDimension.left > parentDimension.width + 10) return positionObj["left"]
      else if (window.innerHeight - (parentDimension.bottom + 10) > parentDimension.height) return positionObj['bottom']
      else return positionObj["top"]
    }
  }

  useEffect(() => {
    if (!showMagnifier || inside) window.removeEventListener('wheel', handelMouseWheel)
  ...
  const imageMagnifier = (
    <div
      ...
      style={{
        ...
        left: inside ? `${x - magnifierWidth / 2}px` : getPosition().left + 'px',
        ...
        backgroundSize: `${imgWidth * (inside ? zoomLevel : magnifierZoomLevel)}px ${imgHeight * (inside ? zoomLevel : magnifierZoomLevel)}px`,

        //calculate position of zoomed image.
        backgroundPositionX: `${-x * (inside ? zoomLevel : magnifierZoomLevel) + (inside ? magnifierWidth : width) / 2}px`,
        backgroundPositionY: `${-y * (inside ? zoomLevel : magnifierZoomLevel) + (inside ? magnifierHeight : height) / 2}px`
      }}
    />
  )
  ...
  return (
    ...
    >
      ...
      />
      {
        showMagnifier ? inside ? imageMagnifier : <PortalComponent>{imageMagnifier}</PortalComponent> : null
      }
      {
        !inside && <div
