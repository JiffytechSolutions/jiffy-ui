...
const CheckboxGroup = ({ title,isClickableFullItem, name, value = [], onChange = () => { }, direction = 'vertical', options, required, helpIcon, controlStates, customClass, ...props }: CheckboxGroupI) => {
    ...
    <div
      ...
      >
      {
        title && <Text
          ...
          customClass={
            required
              ? "inte--required inte-form__label"
              : "inte-form__label"
          }
        >
          {title}
        ...
        {options.map((item, index) => {
          return (
            <li
              ...
              onClick={() => isClickableFullItem && handleCheckboxGroupChange(!value.includes(item.value) , item.value)}
            >
              <Checkbox 
