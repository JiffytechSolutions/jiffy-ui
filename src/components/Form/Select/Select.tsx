...
const Select = ({
  ...
  const handleDetectScrollToBottom = () => {
    const { scrollTop, scrollHeight, clientHeight } =
      dropdownListRef.current.firstChild;
    ...
    if (groupDataPresent) {
      if (event.key === "ArrowUp") {
        ...
        if (
          ...
          if (
            onScrollToBottom &&
            options.length - 1 === nextGroupIndex &&
            (options[nextGroupIndex] as GroupedObjI).group.length - 1 ===
              nextIndex &&
            !isDataLoading
          ) {
            onScrollToBottom();
    ...
    <>
      ...
      >
        ...
        <ul
          ...
          onTouchStart={(e: any) => handleTouchStart(e)}
          {...(onScrollToBottom
            ? { onScroll: handleDetectScrollToBottom }
            : {})}
          {...(isMobile && {
            style: {
              maxHeight:
        ...
        >
          ...
          {onScrollToBottom && isDataLoading && (
            <li className="inte-select__item inte-select__item--loading">
