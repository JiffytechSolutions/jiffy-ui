...
import { ChevronLeft, Plus, X } from '../../../../storybook/Foundation/Icons/Icons';
...
const RangePicker = ({
  ...
  const [calendarType, setCalendarType] = useState<"day" | "year" | "month">('day')
  const [prevCalendarType, setPrevCalendarType] = useState<"day" | "year" | "month">('day')
  ...
  const handelCalendarDisableDate = useCallback((testDate: Date) => {
    if(isDateDisable){
  ...
  const handelDateHover = useCallback((date?: Date | undefined, event?: React.MouseEvent<HTMLTableCellElement, MouseEvent>) => {
    const ele = event?.currentTarget;
    if (ele && !date) {
      ele.classList.remove("inte-calander__cell--rangeStart", "inte-calander__cell--rangeEnd", "inte-calander__cell--rangeSingle")
    }
    if (ele && date && selectedDate[0] && selectedDate[1]) {
      if (currControl === 'start') {
        if (date > selectedDate[1]) ele.classList.add(`inte-calander__cell--rangeSingle`)
        else ele.classList.add('inte-calander__cell--rangeStart')
      }
      else {
        if (date < selectedDate[0]) ele.classList.add('inte-calander__cell--rangeSingle')
  ...
  const isDateInHoverRange = useCallback((currDate: Date) => {
    if (!hoveredDate || isMobile) return false
    if (selectedDate[0] && selectedDate[1]) {
      let t = true
      if (currControl === 'start') {
        if (hoveredDate > selectedDate[1]) t = false
        else if (currDate > selectedDate[1] || currDate < hoveredDate) t = false
      }
      else {
        if (hoveredDate < selectedDate[0]) t = false
        else if (currDate > hoveredDate || currDate < selectedDate[0]) t = false
      }
      return t
    }
  ...
  const isDateInSelectedRange = useCallback((currDate: Date) => {
    if (selectedDate[0] && selectedDate[1] && (selectedDate[0] < currDate) && (selectedDate[1] > currDate)) return true
    return false
  }, [selectedDate])
  ...
  const handelDateClick = useCallback((newDate: Date) => {
    if (currControl === "start") {
      if (calendarType === 'day') {
        if (selectedDate[1] && newDate > selectedDate[1]) {
          !isMobile ? onDateChange([newDate, undefined]) : setSelectedDate([newDate, undefined])
        }
        else {
          !isMobile ? onDateChange([newDate, selectedDate[1]]) : setSelectedDate([newDate, selectedDate[1]])
        }
        if (!selectedDate[1] || newDate >= selectedDate[1]) setCurrControl('end')
      ...
      if (calendarType === 'day') {
        if (selectedDate[0] && newDate < selectedDate[0]) {
          !isMobile ? onDateChange([undefined, newDate]) : setSelectedDate([undefined, newDate])
        }
        else {
          !isMobile ? onDateChange([selectedDate[0], newDate]) : setSelectedDate([selectedDate[0], newDate])
        }
        if (!selectedDate[0] || newDate <= selectedDate[0]) setCurrControl('start')
        else !isMobile && setIsOpen(false)
      }
      else {
        changeCalanderType()
  ...
  const activator = (
    <TextField
      ...
      changeControl={(newControl: 'start' | 'end') => setCurrControl(newControl)}
  ...
  return (
    <div className='inte-textField--forDatePicker'>
      ...
      <Popover
        heading={currControl === 'start' ? "Select Start Date" : "Select End Date"}
        activator={activator}
        isOpen={isOpen}
      ...
      >
        <>
          <div className='inte-rangePicker'>
            {
              (!modalOpen || isMobile) && <CalendarHeader
            ...
            {
              (!modalOpen || isMobile) && <div className={`inte-rangePicker__calanderContainer inte-rangePicker--control-${currControl}`}>
                <Calendar
                ...
                />
                {
                  (!isMobile && calendarType === 'day') && <Calendar
            ...
            }
            {
              (!isMobile && modalOpen) && (
                <TimePicker
                  ...
                  heading={currControl === 'start' ? "Select Start Time" : "Select End Time"}
                />
              )
            }
          </div>
      ...
      {
        helpText && (
          <div className='inte-datePicker__helpText'>{helpText}</div>
        )
      }
    </div>
  )
