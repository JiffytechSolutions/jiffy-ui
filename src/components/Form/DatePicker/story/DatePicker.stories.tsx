...
const Template = ({ ...rest }) => {
  ...
  return <Card>
    <DatePicker
      ...
      textFieldPlaceholder={rest.showTime ? "dd/mm/yyyy, hh:mm AM" : "dd/mm/yyyy"}
    />
  </Card>
}
...
export const datePickerWithDisableDates = ({...rest}) => {
  ...
  const handelIsDateDisable = (testDate:Date) => {
    ...
    if(testDate.getFullYear() <= startDate.getFullYear() && testDate.getMonth() <= startDate.getMonth()) {
      if(testDate.getMonth() === startDate.getMonth())  return testDate.getDate() < startDate.getDate()
      return true
    } 
    return false
  }
  ...
  useEffect(() => {
    if (!isTyping) {
      let res = ''
      if (selectedDate) {
        res += `${make2Words(selectedDate.getDate())}/${make2Words(selectedDate.getMonth() + 1)}/${selectedDate.getFullYear()}`
        if (selectedTime) res += `, ${make2Words(selectedTime.hh)}:${make2Words(selectedTime.mm)} ${selectedTime.meridian}`
      }
