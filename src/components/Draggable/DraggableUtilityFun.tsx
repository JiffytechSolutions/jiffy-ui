...
export const pointInRangeArr = (arr: elementRect[], point: { x: number; y: number }): number => {
    const { x, y } = point;
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (rowDistance < minDistance || (rowDistance === minDistance && colDistance < Math.abs(x - (arr[nearestIndex].left + arr[nearestIndex].width / 2)))) {
